...
\normalsize
\noindent
Title: On-Shell Methods for Tree-Level Amplitudes in (De)Constructed Gauge Theory
...
This thesis studies how to calculate scattering amplitudes in a class of simplified gauge theories known as (De)constructed models, using modern on-shell techniques. In traditional quantum field theory, Feynman diagrams provide a systematic way to compute these amplitudes, but the calculations often become very complicated — especially in gauge theories with many particles or redundant degrees of freedom.

To overcome these difficulties, we use on-shell methods that rely on general principles like momentum conservation, unitarity, and analyticity, rather than directly computing every diagram. A key tool is the BCFW recursion relation, which builds complicated amplitudes from simpler ones by shifting external momenta. We also use the spinor-helicity formalism, which rewrites particle momenta in terms of spinors, making many expressions simpler and more symmetric. To further reduce complexity, we organize our calculations using color ordering, which separates the physics from the gauge group structure.

Our main focus is on the two-site model, a simple setup that mimics extra - dimensional physics using only four-dimensional fields. In this model, we compute various tree-level amplitudes involving both massless and massive gauge bosons. We find interesting simplifications and cancellations, especially in processes involving longitudinal polarization, and confirm that these effects match expectations from higher-dimensional theories. For more complex helicity configurations (beyond MHV), we apply the CSW expansion, which reconstructs amplitudes using MHV building blocks.

Our results show that on-shell techniques are powerful even in models with massive particles and broken symmetries. These compact results hint at deeper geometric structures behind quantum field theory.

Looking forward, this work lays the foundation for further studies, including loop-level calculations, connections to gravity via the double-copy approach, and exploring deeper structures like the CHY formalism, scattering forms, and the amplituhedron, which aim to reformulate quantum field theory in a more elegant and unified way.
...
If we flip one more helicity, the situation becomes quite different. Amplitudes $A_n[1^-,2^-,3^+,\cdots,n^+]$ does not equal to 0 and is quite important called \textbf{MHV amplitudes}.
