...
\begin{frame}{Why We Study Scattering Amplitudes?}
  \begin{enumerate}
    ...
    \begin{itemize}
      \item Amplitudes exhibit hidden symmetries not visible in the Lagrangian
...
\begin{frame}
    ...
    \begin{itemize}
        \item Lorentz covariance $\quad \Rightarrow \quad$ M as a function of Lorentz invariants $p_i\cdot p_j$ , $p_i\cdot\epsilon_j$ and $\epsilon_i\cdot \epsilon_j$.
        \item Analiticity $\quad \Rightarrow \quad$ M is a analytic function of $p_i\cdot p_j$ and $p_i\cdot\epsilon_j$
        \item Unitarity $\quad \Rightarrow \quad$ simple poles $\quad \leftrightarrow \quad$ single particles\\
        \hspace{3.8em}$\quad\Rightarrow\quad$branch cuts $\quad \leftrightarrow\quad$ continuous states
        \item Tree Level $\quad \Rightarrow \quad$ M is a rational function
    \end{itemize}
...
\end{equation*}
This seems to be too many, so people change to use Mandelstam variables $(\text{for}~~\pi\pi~~\text{scattering})$
\begin{equation*}
    s,t,u\quad\text{satisfying}\quad s+t+u=0
\end{equation*}
\end{frame}
...
\begin{frame}
    ...
    \end{figure}
    It means $M(p_1p_2\rightarrow p_3p_4)$ can be transformed to $M(p_1p_2-p_3-p_4)$ by analytic continuation.

    Remember that the definition of helicity is $\textcolor{red}{h = \frac{\vec{S} \cdot \vec{p}}{|\vec{p}|}}$, so the sign will be flipped under crossing symmetry.
\end{frame}

\begin{frame}
    In this report, we choose the convention that all particles are \textcolor{red}{ingoing}.

    The concept “\textbf{MHV} (maximally helicity violating)” comes from thinking a ~$2 \rightarrow n-2$ scattering process.

    By crossing symmetry, an outgoing gluon with 
\(
\left\{
\begin{array}{l}
\text{negative} \\
...
\begin{equation*}
    A[1^+2^+3^-4^-5^+\cdots n^+]\quad \text{crossed over to} \quad 1^-2^-\rightarrow 3^-4^-5^+\cdots n^+
\end{equation*}
This is case in which we can maximally violate the helicity, so called \textbf{MHV amplitudes}.

\textbf{NMHV}(Next to MHV) can be similarly defined by including 3 minus helicity and n-3 plus helicity gluon, like 
\begin{equation*}
    A[1^+2^+3^-4^-5^-6^+\cdots n^+]
\end{equation*}
So, $\mathrm{N^KMHV}$ described amplitudes with $K+2$ negative helicities and $n-K-2$ positive helicities.
\end{frame}
...
\begin{frame}
\frametitle{Old Results from New Perspective} 

\textbf{Parke–Taylor Formula (MHV amplitudes):}
\begin{gather*}
    A_5[1^+2^+3^+4^+5^+] = 0 \\
    A_5[1^-2^+3^+4^+5^+] = 0 \\
    \textcolor{red}{A_5[1^-2^-3^+4^+5^+]} = 
    \frac{\langle 1 2 \rangle^4}
         {\langle 1 2 \rangle \langle 2 3 \rangle \langle 3 4 \rangle \langle 4 5 \rangle \langle 5 1 \rangle}
    \quad (\text{first non-trivial one})
\end{gather*}
\pause
\vspace{-1em}
\begin{gather*}
...
\end{gather*}
\pause
This kind of computation can be reproduced by On-shell method.
    \begin{center}
...
\begin{frame}
    \frametitle{The Introduction to BCFW Recursion Relation}
...
\begin{frame}
    ...
    \begin{gather*}
        ...
        \sket{\hat{i}}=\sket{i}-z\sket{j},\qquad \aket{\hat{i}}=\aket{i}\\
        \sket{\hat{j}}=\sket{j},\qquad \aket{\hat{j}}=\aket{j}+z\aket{i}
    \end{gather*}
    satisfying 
    \begin{equation*}
...
\begin{frame}
    ...
    \end{equation*}
    Here I show the large z behavior for gluon scattering and Einstein gravity 
...
proved by using soft-collinear EFT ~\textcolor{green}{(N.~Arkani-Hamed and J.~Kaplan,
...
\begin{frame}
    ...
    For real momentum:
    \begin{align*}
        &\qquad \qquad \qquad\qquad \avg{12}=([21])^* \\
        &\Rightarrow (p_1+p_2)^2=\cbrak{12}{21}=0\quad \text{only if}\quad \avg{12},[21]=0
    \end{align*}
    If the momentum is \textbf{complexed}, we have 
    \begin{align*}
        &\qquad\qquad\qquad\qquad\qquad\avg{12}\neq[21]^*\\
        &\Rightarrow (p_1+p_2)^2=\mdavg{12}{21}=0 \quad\Longleftrightarrow \quad \avg{12}=0\quad \text{or} \quad [21]=0
    \end{align*}
    Then we can obtain
    \begin{equation*}
        \mathcal{A}\propto \frac{\avg{}}{\avg{}\avg{}}\quad \text{or}\quad \frac{[]}{[][]}
    \end{equation*}
...
\begin{frame}
    ...
    \textbf{Example}: 3-garviton amplitude\\
    \begin{equation*}
        \mathcal{M}_3(1^-\, 2^-\, 3^+) = \frac{\langle 12 \rangle^6}{\langle 23 \rangle^2 \langle 31 \rangle^2} = \left(\frac{\avg{12}^3}{\mdavg{23}{31}}\right)^2=A_3[1^-\, 2^-\, 3^+]^2
    \end{equation*}
...
\begin{frame}
    ...
    Of course, BCFW does not only apply to pure Yang-Mills
    \begin{itemize}
        \item Pure Yang-Mills theory.
        \item Scalar - QED and QCD
        \item Scalar theory $\lambda \phi^4$.
        \item $\mathcal{N} =4$ super Yang - Mills theory.
        \item Einstein Gravity and some modified gravity.
        \item $\mathcal{N}=8$ Supergravity.
        \item $\cdots\cdots$
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{Conjecture about large-z behavior}
    From the 6-point amplitude 
    \begin{equation*}
        A[1^\Phi2^{\Phi^\dagger}3_1^+4_1^+5_2^+6_2^-]=\frac{\asqu{26}\!\asqu{16}}{\textcolor{blue}{\mdavg{23}{34}\!\avg{41}}\!\textcolor{red}{\mdavg{25}{56}\!\avg{61}}}
    \end{equation*}
    we can find that if we choose
    \begin{equation*}
        [2,a^-\rangle\quad \text{shift}:\left\{ 
\begin{array}{l}
1/z\qquad \text{if 2 and a adjacent}\\
1/z^2\qquad \text{if 2 and a not adjacent}
\end{array}
\right.
    \end{equation*}
    similar to different shift
    \begin{equation*}
        [a^-,2\rangle\quad\text{shift}:\left\{
            \begin{array}{l}
                1/z\qquad \text{if 2 and a adjacent}\\
                1/z^2\qquad \text{if 2 and a not adjacent}
            \end{array}
    ...
    From the structure of amplitudes in two site model, it is suggestive to suspect the large-z in n site model
\begin{equation*}
        [1(\text{or 2}),a^-\rangle\quad\text{shift}:\left\{
            \begin{array}{l}
                1/z^{n-1}\qquad \text{if 2 and a adjacent}\\
                1/z^n\qquad \text{if 2 and a not adjacent}
            \end{array}
        \right.
    \end{equation*}
\end{frame}
...
\textcolor{red}{The 3-particle pole $P_{126}^2$ is encoded inside the BCFW channel !}
    ...
    like \textcolor{green}{"A. Hodges, JHEP 1305, 35 (2013) [arXiv:0905.1473 [hep-th]]." }
    \end{itemize}
\end{frame}
