...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Some recent work show that scattering amplitude contains many interesting mathematical structures.
              Actually, this report introduces one of these structures.
    \end{itemize}
\end{frame}
\iffalse
\begin{frame}
...
\begin{frame}
    \begin{align*}
        ...
        &m_5^{(0)}(I|P_5)=0
    \end{align*}
    From examples above, it is easy to see that when both permutations in $m_n^{(0)}(\alpha|\beta)$ are the same, then the answer is a sum over all color-orded
